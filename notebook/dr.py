...
for i in range(len(layer_sizes)-1):
    w_file = f"notebook/w{i+1}.npy"
    b_file = f"notebook/b{i+1}.npy"
...
def save_model():
    for i in range(len(weights)):
        np.save(f"notebook/w{i+1}.npy", weights[i])
        np.save(f"notebook/b{i+1}.npy", biases[i])
...
async def handle_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"Prediction: {digit} ({conf:.2f}% confident)\n\nClass probabilities:\n{prob_str}"
    )

    # Check caption for training
    if update.message.caption:
        caption = update.message.caption.strip()
        if caption.isdigit() and 0 <= int(caption) <= 9:
            label = int(caption)
            y_true = one_hot(np.array([label]))
            loss = train_step(img, y_true, lr=0.01)  # You can adjust learning rate
            save_model()
            await update.message.reply_text(f"Trained on your input! Loss: {loss:.4f}")
        else:
            await update.message.reply_text(
                "Caption ignored: must be a single digit between 0-9 for training."
            )
